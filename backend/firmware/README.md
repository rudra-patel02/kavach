...
- `POST /api/iot/sensor` for ESP32 DHT11/DHT22 temperature and humidity readings

When `DEVICE_SECRET` is set on the backend, send it as `x-device-secret`, a bearer token, or `deviceSecret` in MQTT payloads.

The ESP32 Arduino example posts DHT11 readings every 5 seconds to `/api/iot/sensor`.
