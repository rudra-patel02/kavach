...
## Next
Start **Part 4 — Work Orders & the Closed Loop** (see build-spec.md). Enter plan mode first. Rebuild `models/workOrder.js` + a clean workOrder service/controller/routes: Manager creates+assigns a work order for a machine (link the machine + triggering alert), Engineer advances only their own Open→Assigned→In Progress→Resolved, Viewer can't mutate, whitelisted fields only (no mass-assignment of createdBy/machineId/owner/tenant), at most one active work order per machine, every list scoped to the caller. Then feed resolved work-order repair intervals into the Part 3 KPI MTTR (`computeMTTR` already accepts them via `computeMachineKPIs`'s `workOrders` param).
